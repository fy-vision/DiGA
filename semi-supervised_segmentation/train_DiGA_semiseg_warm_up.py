...
from util.utils import adjust_learning_rate, save_models, create_teacher_params, update_teacher_params, UnNormalize, Normalize
...
# ==== DataLoader ====
city_set_gt   = CityLoader(args.city_data_path, args.data_list_path_city_img_gt, args.data_list_path_city_lbl_gt, max_iters=num_steps* batch_size_ls, crop_size=target_input_size, transform=city_data_aug, mean=IMG_MEAN, set='train')
source_loader= torch_data.DataLoader(city_set_gt, batch_size=batch_size_ls, shuffle=True, num_workers=1, pin_memory=True)

city_set_full_gt   = CityLoader(args.city_data_path, args.data_list_path_city_img_gt, args.data_list_path_city_lbl_gt, max_iters=num_steps* batch_size_hs, crop_size=target_input_size_full, transform=city_data_aug, mean=IMG_MEAN, set='train')
...
for i_iter in range(num_steps):
    ...
    with torch.no_grad():
        _, t_pred1_stu, t_pred2_stu, t_feat_stu = student(tdatav[:,[2, 1, 0],:,:])
    s_pred2_cat_stu = upsample_src(s_pred2_cat_stu)
    s_pred1_cat_stu = upsample_src(s_pred1_cat_stu)
    t_pred2_stu = upsample_tgt(t_pred2_stu)
    ...
    loss_s_distil2 = distillation_loss(s_pred2_cat_tea, s_pred2_cat_stu)
    loss_s_distil1 = distillation_loss(s_pred1_cat_tea, s_pred1_cat_stu)
    loss_distil = loss_s_distil2 + lambda_auxilary*loss_s_distil1

    # visualize segmentation map

    pred_s = F.softmax(s_pred2_stu, dim=1).data.max(1)[1].cpu().numpy()
    pred_t = F.softmax(t_pred2_stu, dim=1).data.max(1)[1].cpu().numpy()
    ...
    total_loss = lambda_seg * loss_semseg + lambda_distil * loss_distil
    ...
    if i_iter%500== 0:
        ...
        imgs_t = torch.cat(((tdatav[:,[2, 1, 0],:,:].cpu()+1)/2, Variable(torch.Tensor((map_t.transpose((0, 3, 1, 2))))), Variable(torch.Tensor((gt_t.transpose((0, 3, 1, 2)))))), 0)
