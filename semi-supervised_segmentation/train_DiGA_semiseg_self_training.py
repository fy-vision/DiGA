...
from util.loader.augmentations import Compose, RandomHorizontallyFlip, RandomCrop, Compose_Pseudo, RandomHorizontallyFlip_Pseudo, RandomSized_and_Crop_Pseudo
from util.metrics import runningScore
from util.loss import cross_entropy2d, distillation_loss
...
parser = argparse.ArgumentParser(description='DiGA semi-supervised semantic segmentation')
...
parser.add_argument('--pseudo_dir', type=str, default=PSEUDO_DIR, help='the path to where the pseudo labels for the unlablled data are saved.')
...
parser.add_argument('--data_list_path_city_img_gt', type=str, default=DATA_LIST_PATH_CITY_IMG_GT)
parser.add_argument('--data_list_path_city_lbl_gt', type=str, default=DATA_LIST_PATH_CITY_LBL_GT)
...
num_classes = 19
source_input_size_full = [1024, 2048]
...
# Setup Augmentations
city_data_aug_gt = Compose([RandomHorizontallyFlip(),
                            RandomCrop([512, 1024])
                            ])

city_data_aug = Compose_Pseudo([RandomHorizontallyFlip_Pseudo(),
                         RandomSized_and_Crop_Pseudo([512, 1024])
                         ])
...
# ==== DataLoader ====
city_set_gt = CityLoader(args.city_data_path, args.data_list_path_city_img_gt, args.data_list_path_city_lbl_gt,
                         max_iters=num_steps * batch_size_ls, crop_size=target_input_size, transform=city_data_aug_gt,
                         mean=IMG_MEAN, set='train')
source_loader = torch_data.DataLoader(city_set_gt, batch_size=batch_size_ls, shuffle=True, num_workers=1, pin_memory=True)

city_set_full_gt = CityLoader(args.city_data_path, args.data_list_path_city_img_gt, args.data_list_path_city_lbl_gt,
                              max_iters=num_steps*batch_size_hs, crop_size=target_input_size_full,
                              transform=city_data_aug_gt, mean=IMG_MEAN, set='train')
source_loader_full = torch_data.DataLoader(city_set_full_gt, batch_size=batch_size_hs, shuffle=True, num_workers=1, pin_memory=True)

city_set = CityLoader(args.city_data_path, args.data_list_path_city_img, args.data_list_path_city_lbl,
                      max_iters=num_steps*batch_size_ls, crop_size=target_input_size, transform=city_data_aug,
                      mean=IMG_MEAN, set='train', use_pseudo=True, pseudo_dir=args.pseudo_dir)
target_loader = torch_data.DataLoader(city_set, batch_size=batch_size_ls, shuffle=True, num_workers=1, pin_memory=True)

city_set_full = CityLoader(args.city_data_path, args.data_list_path_city_img, args.data_list_path_city_lbl,
                           max_iters=num_steps * batch_size_hs, crop_size=target_input_size_full, transform=city_data_aug,
                           mean=IMG_MEAN, set='train', use_pseudo=True, pseudo_dir=args.pseudo_dir)
...
val_set = CityLoader(args.city_data_path, args.data_list_path_val_img, args.data_list_path_val_lbl, max_iters=None,
                     crop_size=[1024, 2048], mean=IMG_MEAN, set='val')
...
student_opt = optim.SGD(student.parameters(), lr=learning_rate_seg, momentum=0.9, weight_decay=weight_decay)
...
upsample_src = nn.Upsample(size=[512, 1024], mode='bilinear', align_corners=True)
upsample_tgt = nn.Upsample(size=[512, 1024], mode='bilinear', align_corners=True)
...
for i_iter in range(num_steps):
    ...
    sdatav_cat = torch.cat([sdatav, sdatav_aug])

    # forwarding
    _, s_pred1_cat_stu, s_pred2_cat_stu, s_feat_cat_stu = student(sdatav_cat[:,[2, 1, 0],:,:])

    s_pred2_stu = s_pred2_cat_stu[:batch_size]
    s_pred2_stu_aug = s_pred2_cat_stu[batch_size:]

    s_pred1_cat_stu = upsample_src(s_pred1_cat_stu)
    s_pred1_stu = s_pred1_cat_stu[:batch_size]

    with torch.no_grad():
        _, s_pred1_cat_tea, s_pred2_cat_tea, s_feat_cat_tea = teacher(sdatav_cat[:,[2, 1, 0],:,:])

    s_pred2_tea_aug_raw = s_pred2_cat_tea[batch_size:]
    s_pred2_cat_tea = upsample_src(s_pred2_cat_tea)

    s_feat_tea_aug = s_feat_cat_tea[batch_size:]
    s_feat_tea = s_feat_cat_tea[:batch_size]

    s_pred2_tea = s_pred2_cat_tea[:batch_size]
    s_pred2_tea_aug = s_pred2_cat_tea[batch_size:]

    s_pred1_cat_tea = upsample_src(s_pred1_cat_tea)

    #bilateral-consensus
    with torch.no_grad():
        tlabelv_pseudo = tlabelv_pseudo_prob.clone()
        _, _, t_pred2_tea, t_feat_tea = teacher(tdatav[:,[2, 1, 0],:,:])
    ...
    with torch.no_grad():
        ...
        vectors_t, ids_t = class_features.calculate_mean_vector(t_feat_tea, t_pred2_tea.detach(), newlabels_t)
        ...
        vectors_s, ids_s = class_features.calculate_mean_vector(s_feat_tea_aug, s_pred2_tea_aug_raw.detach(), newlabels_s)
    ...
    _, cross_pred1_mix, cross_pred2_mix, cross_feat_mix = student(cross_mix[:,[2, 1, 0],:,:])
    cross_pred2_mix = upsample_tgt(cross_pred2_mix)
    cross_pred1_mix = upsample_tgt(cross_pred1_mix)

    # ==== segmentation loss ====
    s_pred2_stu = upsample_src(s_pred2_stu)
    loss_semseg = seg_loss(s_pred2_stu, slabelv) + lambda_auxilary * seg_loss(s_pred1_stu, slabelv)

    # ==== symmetric distillation loss ====
    s_pred2_cat_stu = upsample_src(s_pred2_cat_stu)
    loss_s_distil = distillation_loss(s_pred2_cat_tea, s_pred2_cat_stu) + lambda_auxilary*distillation_loss(s_pred1_cat_tea, s_pred1_cat_stu)

    # ==== segmentation loss pseudo target====
    loss_crossmix_semseg = seg_loss(cross_pred2_mix, crossmix_label) + lambda_auxilary*seg_loss(cross_pred1_mix, crossmix_label)
    loss_semseg += loss_crossmix_semseg

    # visualize segmentation map
    pred_s = F.softmax(s_pred2_stu, dim=1).data.max(1)[1].cpu().numpy()
    pred_t = F.softmax(cross_pred2_mix, dim=1).data.max(1)[1].cpu().numpy()

    s_pred2_stu_aug = upsample_src(s_pred2_stu_aug)
    pred_s_aug = F.softmax(s_pred2_stu_aug, dim=1).data.max(1)[1].cpu().numpy()

    map_s  = city_set.decode_segmap(pred_s)
    map_t  = city_set.decode_segmap(pred_t)
    map_s_aug  = city_set.decode_segmap(pred_s_aug)
    ...
    if i_iter%500== 0:
        imgs_s = torch.cat(((sdatav[:,[2, 1, 0],:,:].cpu()+1)/2, Variable(torch.Tensor((map_s.transpose((0, 3, 1, 2))))), (sdatav_aug[:,[2, 1, 0],:,:].cpu()+1)/2, Variable(torch.Tensor((map_s_aug.transpose((0, 3, 1, 2))))), Variable(torch.Tensor((gt_s.transpose((0, 3, 1, 2)))))), 0)
    ...
    if i_iter % num_calmIoU == 0 and i_iter > 0:
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(val_loader)):
            ...
            image_ds = nn.functional.interpolate(images_val[:,[2, 1, 0],:,:], (512, 1024), mode='bilinear', align_corners=True)
            with torch.no_grad():
                _, _, pred, _ = student(images_val[:,[2, 1, 0],:,:])
