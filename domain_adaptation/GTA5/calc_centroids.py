...
if __name__ == "__main__":
    ...
    gta5_data_aug = Compose([RandomHorizontallyFlip(),
                         RandomSized_and_Crop([720, 1280])
                         ])
    gta5_set = GTA5Loader(args.gta5_data_path, args.data_list_path_gta5, max_iters=None,
                          crop_size=source_input_size, transform=gta5_data_aug, mean=IMG_MEAN)
    source_loader = torch_data.DataLoader(gta5_set, batch_size=batch_size, shuffle=False, num_workers=1,
                                          pin_memory=True)

    gta5_set_full   = GTA5Loader(args.gta5_data_path, args.data_list_path_gta5, max_iters=num_steps* batch_size, crop_size=source_input_size_full, transform=gta5_data_aug, mean=IMG_MEAN)
    source_loader_full = torch_data.DataLoader(gta5_set_full, batch_size=batch_size, shuffle=False, num_workers=1, pin_memory=True)
