...
num_classes = 19
source_input_size_full = [1052, 1914]
...
# ==== DataLoader ====
gta5_set   = GTA5Loader(args.gta5_data_path, args.data_list_path_gta5, max_iters=num_steps* batch_size_ls, crop_size=source_input_size, transform=gta5_data_aug, mean=IMG_MEAN)
source_loader= torch_data.DataLoader(gta5_set, batch_size=batch_size_ls, shuffle=True, num_workers=1, pin_memory=True)

gta5_set_full   = GTA5Loader(args.gta5_data_path, args.data_list_path_gta5, max_iters=num_steps* batch_size_hs, crop_size=source_input_size_full, transform=gta5_data_aug, mean=IMG_MEAN)
source_loader_full = torch_data.DataLoader(gta5_set_full, batch_size=batch_size_hs, shuffle=True, num_workers=1, pin_memory=True)
...
for i_iter in range(num_steps):
    ...
    sdatav_aug = 0.4*norm(extra_aug(sdatav))+0.6*sdatav
    ...
    map_s_aug  = gta5_set.decode_segmap(pred_s_aug)
    ...
    if i_iter % num_calmIoU == 0 and i_iter>0:
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(val_loader)):
            ...
            with torch.no_grad():
                ...
                _,_,pred_ds, _ = student(image_ds)
